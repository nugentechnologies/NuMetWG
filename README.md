...
Figure 4: TMWG-v2 Bioinformatics workflow including estimated compute time as percentage of total. Table 2: Table of all steps for TMWG-v2 analysis including recommended software and website.
...
Please refer to the Pre-alignment tools section of this document for ways to validate
...
Input file(s): 	SAMPLE_R1.fastq.gz, SAMPLE_R2.fastq.gz Output files(s):	 SAMPLE_R1.seqtk_sample.1000000.fastq.gz, SAMPLE_R2.seqtk_sample.1000000.fastq.gz
Tool(s):	seqtk


$    seqtk    sample    -s987654321    SAMPLE_R1.fastq.gz    1000000   | gzip    -c  >
...
$    seqtk    sample    -s987654321    SAMPLE_R2.fastq.gz    1000000   | gzip    -c  >
SAMPLE_R2.seqtk_sample.1000000.fastq.gz


This will produce the SAMPLE_R1.seqtk_sample.1000000.fastq.gz and
SAMPLE_R2.seqtk_sample.1000000.fastq.gz files.
...
-	Given  the  fastq.gz  files  produced  by  Illumina  sequencing,  the  quality  of  the sequencing data can be assessed using FastQC. Use cases are detailed below in Table3. Instructions on how to install this software tool and dependencies can be found at:
...
Input file(s): 	SAMPLE_R1.fastq.gz, SAMPLE_R2.fastq.gz Output files(s): SAMPLE_R1_fastqc.zip, SAMPLE_R2_fastqc_zip Tool(s):	 fastqc
...
Given input files that have passed sequencing QC, they should then be filtered for sequencing quality (Phred score) and undergo adapter removal. This can be achieved with sequence trimming software, such as cutadapt. Instructions on how to install this software and its dependencies can be found at:
http://github.com/marcelm/cutadapt/releases
...
Command line arguments to run cutadapt


Input file(s):        SAMPLE_R1.fastq.gz, SAMPLE_R2.fastq.gz
AGATCGGAAGAGC, AGATCGGAAGAGC (adapter1 and adapter2) Output files(s): SAMPLE_R1.trimmed.fq.gz, SAMPLE_R2.trimmed.fq.gz Tool(s):	cutadapt


$   cutadapt  –q   20   –m  15   -a AGATCGGAAGAGC   -A  AGATCGGAAGAGC   -o SAMPLE_R1.trimmed.fq.gz -p SAMPLE_R2.trimmed.fq.gz SAMPLE_R1.fastq.gz SAMPLE_R2.fastq.gz   2>&1 SAMPLE.trimmed.log
This will produce the SAMPLE_R1.trimmed.fq.gz and SAMPLE_R2.trimmed.fq.gz files. Following this step, the TMWG datasets need to be processed with a second cutadapt step,
in order to trim the last 10 bases of Read1 (3’ end) and the first 10 bases of Read2 (5’ end)
as required for TMWG:


Input files(s):      SAMPLE_R1.trimmed.fq.gz, SAMPLE_R2.trimmed.fq.gz
Output file(s):      SAMPLE_R1.cut.fq.gz, SAMPLE_R2.cut.fq.gz
Tool(s):                        cutadapt


$ cutadapt  –m  15  –u  -10 –U 10  –o  SAMPLE_R1.cut.fq.gz –p  SAMPLE_R2.cut.fq.gz
SAMPLE_R1.trimmed.fq.gz SAMPLE_R2.trimmed.fq.gz 2>&1 SAMPLE.cut.log


NOTE: the TMWG-v1 (version1) of the protocol requires trimming of  -u -6 -U 6 instead of
-u -10 -U 10.


The      cutadapt       application     will     produce     the     SAMPLE_R1.cut.fq.gz     and
SAMPLE_R2.cut.fq.gz files, which are then ready to be used for alignment.
...
1.  Genome preparation: process genome reference file and build bowtie indices for use by the Bismark alignment strategy.
2.  Alignment:  run  bismark   with  the  trimmed  sequences  against  the  converted reference that matches the sample.
...
Input file(s): 	/path/to/fasta/human/GRCh38/*.fa
Output folder(s):
/path/to/fasta/human/GRCh38/Bisulfite_genome/GA_conversion
/path/to/fasta/human/GRCh38/Bisulfite_genome/CT_conversion
Tool(s):	bismark_genome_preparation


$ bismark_genome_preparation /path/to/fasta/homo_sapiens/dna/GRCh38


Alignment step:


Input file(s): 	SAMPLE_R1.cut.fq.gz, SAMPLE_R2.cut.fq.gz
Input folder(s):   /path/to/fasta/human/GRCh38
Output file(s): SAMPLE_R1.cut.fq.gz_bismark_bt2_pe.bam SAMPLE_R1.cut.fq.gz_bismark_bt2_PE_report.txt
Tool(s):	bismark


$    bismark    --bam    --maxins    500    /path/to/fasta/homo_sapiens/dna/GRCh38    -1
SAMPLE_R1.cut.fq.gz -2 SAMPLE_R2.cut.fq.gz


The txt file produced in this step contains a report of the bismark   alignment step.
 

The bam file produced in this step describes the alignment of each read (or read pair) to the reference. The reads in this bam file are unsorted and may contain duplicates. This file will be referred to as SAMPLE.bam   hereon for simplicity.


Note: The alignment step is computationally intensive and RAM should be sufficient to accommodate  roughly  3  times  the  size  of  the reference sequence in base pairs. For example, for the alignment against the human reference, more than 10.5GB of RAM is
required. For further guidelines refer to the Bismark user guide.
...
The analysis of fragment sizes in the aligned data can be achieved by running Picard CollectInsertSizeMetrics  on the aligned reads of a given sample, present in the SAMPLE.bam file. Instructions on how to install Picard and its dependencies can be found at: http://broadinstitute.github.io/picard
...
Output file(s): 	SAMPLE_picard_insert_size_metrics.txt
SAMPLE_picard_insert_size_plot.pdf
Tool(s):	Picard CollectInsertSizeMetrics


$ 	java 	–Xmx4g 	-XX:+UseG1GC 	-XX:ParallelGCThreads=2 	-jar
/path/to/Picard/picard.jar 	CollectInsertSizeMetrics 	INPUT=SAMPLE.bam OUTPUT=SAMPLE_picard_insert_size_metrics.txt HISTOGRAM_FILE=SAMPLE_picard_insert_size_plot.pdf METRIC_ACCUMULATION_LEVEL=ALL_READS



The pdf output plot will show a histogram of fragment insert sizes. The txt output file will show the resulting metrics as described here:
...
Command line arguments to run samtools or deduplicate_bismark

Samtools:


Input file(s): 	SAMPLE.bam Output files(s): SAMPLE.rmdup.nsorted.bam Tool(s):	 samtools
...
$ samtools sort -n SAMPLE.rmdup.bam  SAMPLE.rmdup.nsorted.bam


$ samtools fixmate -r SAMPLE.rmdup.nsorted.bam SAMPLE.rmdup.nsorted.fm.bam


$ samtools view -f 0x01 -b SAMPLE.rmdup.nsorted.fm.bam >
SAMPLE.rmdup.nsorted.fm.flt.bam


Using samtools  in a five-step process will (1) sort the reads by chromosomal reference coordinates and then (2) use the output sorted file,  SAMPLE.sorted.bam, as input for the remove duplicates analysis. This will produce a  SAMPLE.rmdup.bam sorted by chromosomal reference coordinates. This file can then be used to (3) re-sort by read name, (4) fix mate cleanup and (5) pair-endness cleanup step. This will produce the file SAMPLE.rmdup.nsorted.fm.flt.bam that will be ready for methylation counting.


deduplicate_bismark:


Input file(s): 	SAMPLE.bam Output files(s):   SAMPLE.deduplicated.bam Tool(s):	 deduplicate_bismark
...
Given  an  aligned  dataset  produced  by  bismark,  e.g.  SAMPLE.bam,  the  tool deduplicate_bismark can be used to remove or mark the duplicate reads in one single step. The tool deduplicate_bismark will not require pre-sorting of the bam file and produces a deduplicated read name sorted output file that is ready to be used in the downstream methylation counting step.
...
For customers with access to a compute resource with enough RAM memory, the one-step
deduplicate_bismark   tool is recommended.
...
Pre-alignment,   Alignment   and   Post-alignment   Paired-End
Report for single or multiple samples
...
Estimation of hydroxymethylation and methylation levels from BS (BS)
and oxBS methylation counts
...
Visualising 5hmC and 5mC profiles in various genome browsers (e.g. IGV, UCSC, WashU
1
Epigenome Browser, Gencode Biodalliance)   can be achieved by converting the MLML
output to both BedGraph and BigWig files. A BedGraph file must first be generated and then converted to a BigWig file.
